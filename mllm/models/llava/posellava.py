...
from mllm.models.constants.llava_constants import IMAGE_TOKEN_INDEX, POSE_TOKEN_INDEX, IGNORE_INDEX
from mllm.utils.mm_utils import get_anyres_image_grid_shape , unpad_image
from .multimodal_encoder.builder import build_vision_tower, build_pose_tower
from .multimodal_projector.builder import build_vision_projector, build_pose_projector

 
class PoseLlavaConfig(LlamaConfig):
    model_type = "posellava"


class PoseLlavaLlamaModel(LlamaModel):
    config_class = PoseLlavaConfig

    def __init__(self, config: LlamaConfig):
        super(PoseLlavaLlamaModel, self).__init__(config)
        ...
        self.pose_projector = build_pose_projector(config)
...
class PoseLlavaForCausalLM(LlamaForCausalLM):
    config_class = PoseLlavaConfig
    supports_report_metrics: bool = True # IMPORTANT
    def __init__(self, config):
        super(LlamaForCausalLM, self).__init__(config)

        self.model = PoseLlavaLlamaModel(config)
        ...
        self.pose_token_idx = POSE_TOKEN_INDEX
    ...
    def get_pose_token_id(self):
        return self.pose_token_idx
    ...
    def get_pose_tower(self):
        return self.model.get_pose_tower()
    ...
    def encode_poses(self, smpl_poses):
        pose_features = self.get_pose_tower()(smpl_poses)
        pose_features = self.model.pose_projector(pose_features)
    ...
    def forward(
        ...
        input_smpl_values: Optional[torch.FloatTensor] = None, # Add this line
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        # encoder pose features
        pose_encoder = self.get_pose_tower()
        if pose_encoder is not None:
            pose_features = self.encode_poses(smpl_pose_params)
        else:
            pose_features = None
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            aux_token_indices = torch.where((cur_input_ids == img_token_idx) | (cur_input_ids == pose_token_idx))[0].tolist()
            num_multi_data = len(aux_token_indices)

            pose_token_indices = torch.where(cur_input_ids == pose_token_idx)[0].tolist()
            pose_modality_flag = [1 if item in pose_token_indices else 0 for item in aux_token_indices]
            num_poses = len(pose_token_indices)
            num_images = num_multi_data - num_poses

            if num_multi_data == 0:
                cur_image_features = image_features[cur_image_idx]
                cur_pose_features = pose_features[cur_pose_idx]
                cur_input_embeds_1 = self.get_model().embed_tokens(cur_input_ids)
                cur_input_embeds = torch.cat([cur_input_embeds_1, cur_image_features[0:0], cur_pose_features[0:0]], dim=0)
            ...
            special_token_indices = [-1] + aux_token_indices +[cur_input_ids.shape[0]]
            cur_input_ids_noim = []
            ...
            for i in range(num_multi_data + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_im[i])
                cur_new_labels.append(cur_labels_noim[i])
                if i < num_multi_data:
                    if pose_modality_flag[i]==1:
                        cur_features = pose_features[cur_pose_idx]
                        cur_pose_idx += 1
                    else:
                        cur_features = image_features[cur_image_idx]
                        cur_image_idx += 1
                    cur_new_input_embeds.append(cur_features)
                    cur_new_labels.append(torch.full((cur_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
            
            if num_images != 0 and num_poses==0:
                cur_features = pose_features[cur_pose_idx]
                cur_pose_idx +=1
                cur_new_input_embeds.append(cur_features[0:0])
            elif num_poses != 0 and num_images==0:
                cur_features = image_features[cur_image_idx]
                cur_image_idx +=1
                cur_new_input_embeds.append(cur_features[0:0])
        ...
        assert cur_pose_idx == pose_features.shape[0]
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None,
        ...
        input_smpl_values = kwargs.pop("input_smpl_values", None)
        ...
        if input_smpl_values is not None:
            inputs['input_smpl_values'] = input_smpl_values
    ...
    def generate(
        ...
        input_smpl_values: Optional[torch.Tensor] = None,
        ...
        **kwargs,
        ...
        if pixel_values is not None or input_smpl_values is not None: 
            (
                inputs,
                position_ids,
                attention_mask,
                ...
                None,
                pixel_values,
                smpl_pose_params =input_smpl_values,
    ...
    @torch.no_grad()
    def chat(self, input_ids: Optional[torch.Tensor] = None,
        pixel_values: Optional[torch.Tensor] = None,
        input_smpl_values: Optional[torch.Tensor] = None,
        image_sizes: Optional[torch.Tensor] = None,
        **kwargs,):
        ...
        if input_smpl_values is None:
            input_smpl_values= torch.zeros((1, 24, 6), dtype= self.model.dtype, device=input_ids.device)
...
AutoConfig.register("posellava", PoseLlavaConfig)   
AutoModelForCausalLM.register(PoseLlavaConfig, PoseLlavaForCausalLM)