...
import json
from mllm.models.llava import PoseChatForCausalLM
...
@register_loader("posechat")
class PoseChatModelLoader(BaseModelLoader):
    def load(self, load_lora_model: bool = False, 
        ...
        if not load_lora_model: 
            model = PoseChatForCausalLM.from_pretrained(
                self.model_local_path, 
                **self.loading_kwargs,
            )
        else:
            ...
            base_model = PoseChatForCausalLM.from_pretrained(base_model_path,
