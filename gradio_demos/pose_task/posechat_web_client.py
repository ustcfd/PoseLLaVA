...
from gradio_demos.api.posechat_server import PoseChatAPI 
...
def http_bot(state, temperature, top_p, max_new_tokens, request: gr.Request):
    ...
    try:
        ...
        response = requests.post(model_api,
            headers=headers, json=pload, stream=True, timeout=10)
        # for line in response.iter_content(5000):
        #     if line:
        #         print(line.decode("utf-8"))
        # for chunk in response.iter_lines(decode_unicode=False, delimiter=b"\0"):
        #     print("VVVVVVVVVVVVVVVVVVVVVVVVVVVVVV:", chunk)
        pose_img_path = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_family_id", type=str, default="posechat")
    ...
    PORT = api_cfg[model_family_id]['port']
    IP_ADDR = api_cfg[model_family_id]['ip']
    model_api = f"http://{IP_ADDR}:{PORT}/predict"
